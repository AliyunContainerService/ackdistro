...
helm_install_hybridnet() {
  for i in `seq 1 3`;do
    sleep 1
    helm -n kube-system upgrade -i --reuse-values $1 chart/$1 -f /tmp/ackd-helmconfig.yaml --set init=null --set daemon.enableFelixPolicy=true --set typha.serverPort=5473 --set images.hybridnet.image=ecp_builder/hybridnet --set images.hybridnet.tag=v0.8.2 && return 0
  done
  return 1
}
...
if [ "${Network}" == "calico" ];then
  ...
  if helm -n kube-system status hybridnet &>/dev/null;then
    ...
    if [ "${HasRecreateOldHybridnet}" == "true" ] && [ "${UpgradeHybridnet}" == "true" ];then
      ...
      for i in `seq 1 3`;do
        sleep 1
        helm -n kube-system upgrade -i --reuse-values hybridnet chart/hybridnet -f /tmp/ackd-helmconfig.yaml --set init=null --set daemon.enableFelixPolicy=true --set typha.serverPort=5473 --set images.hybridnet.image=ecp_builder/hybridnet --set images.hybridnet.tag=v0.8.2 --set defaultIPRetain=false && break
      done
      if [ $? -ne 0 ]; then
